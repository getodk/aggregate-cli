...
import org.opendatakit.aggregateupdater.reused.Pair;
...
public class Request<T> {
  ...
  public static Request<List<Map<String, Object>>> getJsonList(URL url) {
    return new Request<>(GET, url, Optional.empty(), Request::readJsonList, emptyList());
  }

  private static List<Map<String, Object>> readJsonList(InputStream is) {
    try {
      TypeReference<List<Map<String, Object>>> listOfObjects = new TypeReference<List<Map<String, Object>>>() {
      };
      return new ObjectMapper().readValue(is, listOfObjects);
    } catch (IOException e) {
